...
```{r}
train_data <- ovun.sample(inclinacion_peligrosa ~ ., data = data, method = "under", seed = 2023)$data  # default p = 0.5
...
train_data <- ovun.sample(inclinacion_peligrosa ~ ., data = data, method = "over", p = 0.25, seed = 2023)$data
...
write.csv(result_dataset, "data/data-parte-b/submission_10.csv", row.names = FALSE)
```
...
xgb_model <- xgb.train(param, xgb_train, nrounds = 1000) # early_stopping_rounds = 100,  watchlist para  usarlos hace falta las rts de test (creo???)
...
predictions_xgb_numeric <- as.numeric(as.character(prediction_xgb))

result_dataset <- data.frame(id = test_data$id, inclinacion_peligrosa = predictions_xgb_numeric)
