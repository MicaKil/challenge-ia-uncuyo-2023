...
secciones_peligrosas <- data %>%
    group_by(seccion, nombre_seccion) %>%  # agrupar por sección y nombre de sección
...
ggplot(especies_peligrosas, aes(x = reorder(especie, -peligrosidad), y = peligrosidad, fill = reorder(especie, -peligrosidad))) +
    ...
    scale_fill_manual(values = rainbow(length(unique(especies_peligrosas$especie)))) + # Colores aleatorios
